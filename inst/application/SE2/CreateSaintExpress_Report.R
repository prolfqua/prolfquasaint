...
REPORTDATA$FCthreshold <- as.numeric(
  yml$application$parameters$`22|FCthreshold`
)
REPORTDATA$FDRthreshold <- as.numeric(
  yml$application$parameters$`21|BFDRsignificance`
)
...
annotation <- dplyr::mutate(
  annotation,
  raw.file = gsub("\\.raw|\\.d\\.zip", "", raw.file)
)
...
pdata <- pdata |>
  dplyr::filter(combined.total.peptides >= REPORTDATA$nrPeptides)
# configure prolfqua
...
lfqdata$data <- lfqdata$data |>
  dplyr::filter(!grepl("^REV_|^CON_|^zz", protein_Id, ignore.case = TRUE))
...
resSaint <- prolfquasaint::runSaint(
  localSAINTinput,
  spc = REPORTDATA$spc,
  CLEANUP = FALSE
)
...
writexl::write_xlsx(
  RESULTS,
  path = file.path(ZIPDIR, paste0(treat, "_data.xlsx"))
)
...
tmp <- prolfqua::get_uniprot_id_from_fasta_header(
  REPORTDATA$pups$data,
  "protein_Id"
)
...
sigg <- sig |> dplyr::group_by(Bait) |> tidyr::nest()
if (nrow(sigg) > 0) {
  for (i in seq_len(nrow(sigg))) {
...
resContr <- resContrasts |> dplyr::group_by(Bait) |> tidyr::nest()
if (nrow(resContr) > 0) {
  for (i in seq_len(nrow(resContr))) {
    tmp <- resContr$data[[i]]
    tmp$firstID <- sapply(tmp$Prey, function(x) {
      strsplit(x, ";")[[1]][1]
    })
    tmp <- prolfqua::get_uniprot_id_from_fasta_header(tmp, idcolumn = "firstID")
    filename <- paste0("Bait_", resContr$Bait[i], ".rnk")
    write.table(
      data.frame(tmp$UniprotID, tmp$log2_EFCs),
      file = file.path(ZIPDIR, filename),
      col.names = FALSE,
      row.names = FALSE,
      quote = FALSE,
...
text <- c(
  ...
  if (SEP$spc) {
    "total number of PSMs in support of the protein identification"
  } else {
    "normalized protein intensity using the sequences after razor assignment"
  },
  paste0(
    ". For more information about the _combined_protein.tsv_ file see ",
    "[FragPipe output tutorial]( ",
...
rmarkdown::render(
  "SaintExpressReportMsFragger.Rmd",
...
file.copy(
  "SaintExpressReportMsFragger.html",
