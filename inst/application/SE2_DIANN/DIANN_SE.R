...
annot <- annot |>
  dplyr::mutate(
    raw.file = gsub("^x|.d.zip$|.raw$", "", (basename(annot$Relative.Path)))
  )
annotation <- annot
...
res <- prolfquasaint::dataset_set_factors_deprecated(
  atable,
  peptide,
  SAINT = TRUE
)
...
writexl::write_xlsx(
  ...
  path = file.path(
    ZIPDIR,
    paste0(treat, "WU", BFABRIC$workunitID, "_data.xlsx")
  )
)
...
REPORTDATA$prot_annot <- dplyr::rename(
  prot_annot$row_annot,
  protein = protein_Id
)
...
tmp$UniprotID <- sapply(tmp$protein_Id, function(x) {
  strsplit(x, ";")[[1]][1]
})
...
if (nrow(sigg) > 0) {
  for (i in seq_len(nrow(sigg))) {
    tmp <- sigg$data[[i]]
    tmp$UniprotID <- sapply(tmp$Prey, function(x) {
      strsplit(x, ";")[[1]][1]
    })
...
# delete all variables not needed for rendering
rm(list = setdiff(ls(), c("REPORTDATA", "ZIPDIR", "treat", "yml", "BFABRIC")))
SEP <- REPORTDATA
...
text <- c(
  ...
  if (fragPipeDIA) {
    "FragPipe-DIA proteomics pipeline [@yu2023analysis]."
  } else {
    "DIA-NN software [@demichev2020dia]."
  },
  paste0(
...
fileNameHTML <- paste0(
  "SaintExpressReportMsFragger_WU",
  BFABRIC$workunitID,
  ".html"
)
...
file.copy(fileNameHTML, file.path(ZIPDIR, fileNameHTML), overwrite = TRUE)
